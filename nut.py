...
def ups_online_status(status: UPSStatus):
    if status in [UPSStatus.online, UPSStatus.online_low_battery, UPSStatus.online_low_voltage, UPSStatus.online_charge, UPSStatus.online_charge_low_battery, UPSStatus.online_discharge, UPSStatus.online_discharge_low_battery]:
        return UPSOnlineStatus.online
    elif status in [UPSStatus.battery_discharge, UPSStatus.battery_discharge_low_battery, UPSStatus.battery_low_battery]:
...
def ups_charge_status(status: UPSStatus):
    ...
    elif status in [UPSStatus.battery_discharge, UPSStatus.battery_discharge_low_battery, UPSStatus.online_discharge, UPSStatus.online_discharge_low_battery]:
...
def ups_other_status(status: UPSStatus):
    ...
    elif status in [UPSStatus.online_low_battery, UPSStatus.online_charge_low_battery, UPSStatus.online_discharge_low_battery, UPSStatus.battery_discharge_low_battery, UPSStatus.battery_low_battery]:
...
def update_display(full = False):
    ...
    draw_segments = (float(upsBatteryCharge)/100) * segment_count

    i = 0
    while i < draw_segments - 1:
        rect_index = i + 1
        draw.rectangle([(segment_x, segment_max - (i * spacing) - (rect_index * segment_height)), (segment_width, segment_max - (i * spacing) - (i * segment_height))], fill = 0)
        i += 1
    ...
    # Status
    status = UPSStatus(upsStatusCode)
    ...
    draw.text((text_offset, font + text_spacing), upsBatteryCharge + battery_status_text, font = font20, fill = 0)
